...
"""
-------------------------------------------------
    File Name:      Crawler__qyxg_xzxk__credit_cq.py
    Description:    双公示-信用重庆-行政许可爬虫
    Author:         crazy_jacky
    Date:           2018-01-03
    version:        v.1.0
-------------------------------------------------
...
class Crawler__qyxg_xzxk__credit_cq(SpiderAll):
    """双公示-信用重庆-行政许可爬虫"""
    name = 'Crawler__qyxg_xzxk__credit_cq'
    start_urls = ['http://www.xycq.gov.cn/html/query/agree/list.html']
    model_url = 'http://www.xycq.gov.cn/html/query/agree/detail.html?ids={}'
    ...
    def parse(self, response):
        ...
        try:
            ...
            is_end = response.xpath('.//td[@colspan="5"]')
            if not is_end:
                page_num += 1
